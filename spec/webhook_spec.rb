...
module Trello
  describe Webhook do
    ...
    before(:each) do
      allow(client).to receive(:get).with("/webhooks/1234", {}).and_return webhook_payload
    end
    ...
    context "finding" do
      ...
      it "delegates to Trello.client#find" do
        expect(client).to receive(:find).with(:webhook, '1234', {})
        Webhook.find('1234')
      end
    ...
    context "creating" do
      ...
      it 'creates a new webhook and saves it on Trello', refactor: true do
        ...
        expect(client).to receive(:post).with("/webhooks", expected_payload).and_return result
    ...
    context "updating" do
      it "updating description does a put on the correct resource with the correct value" do
        ...
        expect(client).to receive(:put).once.with("/webhooks/#{webhook.id}", expected_payload)
    ...
    context "deleting" do
      it "deletes the webhook" do
        expect(client).to receive(:delete).with("/webhooks/#{webhook.id}")
        webhook.delete
      end
    end
