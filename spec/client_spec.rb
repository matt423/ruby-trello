...
describe Client, "and how it handles authorization" do
  ...
  before do
    allow(TInternet).to receive(:execute).and_return fake_ok_response
    allow(Authorization::AuthPolicy).to receive(:new).and_return(auth_policy)
    allow(auth_policy).to receive(:authorize) do |request|
      request
    end
  end

  it "authorizes before it queries the internet" do
    expect(auth_policy).to receive(:authorize).once.ordered
    expect(TInternet).to receive(:execute).once.ordered
  ...
  it "queries the internet with expanded earl and query parameters" do
    ...
    expect(TInternet).to receive(:execute).once.with expected_request
  ...
  it "raises an error when response has non-200 status" do
    ...
    allow(TInternet).to receive(:execute).and_return response_with_non_200_status

    expect { client.get "/xxx" }.to raise_error expected_error_message
  end

  it "uses version 1 of the API" do
    expect(TInternet).to receive(:execute).once do |request|
  ...
  it "supports post" do
    expect(TInternet).to receive(:execute).once.and_return fake_ok_response
