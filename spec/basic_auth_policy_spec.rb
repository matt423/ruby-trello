...
describe BasicAuthPolicy do
  ...
  it 'adds key and token query parameters' do
    ...
    expect(the_query_parameters['key']).to be === 'xxx_developer_public_key_xxx'
    expect(the_query_parameters['token']).to be === 'xxx_member_token_xxx'
  end
  ...
  it 'preserves headers' do
    ...
    expect(authorized_request.headers).to be === request.headers
  end
