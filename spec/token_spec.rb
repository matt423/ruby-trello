...
module Trello
  describe Token do
    ...
    before(:each) do
      allow(client).to receive(:get).with('/tokens/1234', {}).and_return token_payload
    end
    ...
    context 'finding' do
      ...
      it 'delegates to Trello.client#find' do
        expect(client).to receive(:find).with(:token, '1234', {webhooks: true})
        Token.find('1234')
      end
    ...
    context 'attributes' do
      ...
      it 'has a permission grant' do
        expect(token.permissions.count).to be 3
      end
    end
    ...
    context 'members' do
      it 'retrieves the member who authorized the token' do
        ...
        allow(Trello.client).to receive(:get).with('/members/abcdef123456789123456789', {}).and_return user_payload
        allow(client).to receive(:get).with('/tokens/1234', {}).and_return token_payload
